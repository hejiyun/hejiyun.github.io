...
<template>
    <div class="google-recaptcha">
        <div class="g-recaptcha" 
            data-sitekey="6LeuKq4lAAAAAEXwomiDjOQ0X-7NF0AhwiBOnU6T" 
...
<script setup>
    ...
    onMounted(() => {
        ...
        script.value.src="https://www.recaptcha.net/recaptcha/api.js?render=6LeuKq4lAAAAAEXwomiDjOQ0X-7NF0AhwiBOnU6T"
        document.body.appendChild(script.value);
    })
    //调用人机验证服务测试token
    const onClick = () => {
        grecaptcha.ready(() => {
            grecaptcha.execute('6LeuKq4lAAAAAEXwomiDjOQ0X-7NF0AhwiBOnU6T', {action: 'submit'}).then(async (token) => {
    ...
    const test = async (token) => {
        ...
        const res =  await POST(`https://www.google.com/recaptcha/api/siteverify?secret=6LeuKq4lAAAAAHLK0rGVHzxa5nuFK9WHLyxpFUuG&response=${token}`,null)
