...
<template>
    <div class="main-nav" ref="mainNav">
        <nav class="main-content">
            ...
            <div class="links">
<!--                <a class="text-link" href="https://artexchange.gitbook.io/whitepaper-v0.1/"  target="_blank">WHITEPAPER</a>-->
                <div class="main-menu-box">
                    <div class="main-menu-layer">
                        <div class="menu-close-btn" @click.stop="state.menuclose = !state.menuclose">
                            <icon-menu :class="{close:!state.menuclose}"></icon-menu>
                        </div>
                        <div class="main-menu-list" :class="state.menuclose?'close':'open'" @touchmove.stop.prevent>
                            <router-link v-for="(val,key) in state.routes" @click.stop="state.menuclose=true" :to="val">{{val.meta.navName}}</router-link>
                        </div>
                    </div>
                </div>
                ...
                <template v-if="!userStore.userinfo.access_token">
                    <template v-if="isDesktop">
                        <googleRecaptcha>
                            ...
                            <wallet-list-modal ref="walletList" @selectWallet="selectWallet"></wallet-list-modal>
                        </googleRecaptcha>
                    </template>
                    <wallet-connect ref="wcWallet" v-else @sendLoginStatus="sendLoginStatus" @connectWallet="connectWalletCallback"></wallet-connect>
                </template>
                <div class="social">
                    <span class="headimg" @click="setUserBoxShow('auto')" v-if="userStore.userinfo.pfid">
                        <img :src="imgResize(userStore.userinfo.headimg,150,150,true, true)">
                    </span>
                    <a v-else href="javascript:void(0);" @click.stop="onConnect"><wallet></wallet></a>
                    <i></i>
                    <a href="https://discord.gg/yQ29mSE7MZ" target="_blank"><discord></discord></a>
...
const connectWalletCallback= async (type,obj)=>{
    const res = await window.grTest()
