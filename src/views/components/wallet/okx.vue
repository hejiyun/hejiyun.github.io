...
// ethereum.isOKExWallet=false;
// ethereum.isOkxWallet=false;
// okxwallet.isOKExWallet=false;
// okxwallet.isOkxWallet=false;

// console.log(JSON.stringify(ethereum))
onMounted(() => {
    // Object.defineProperty(window, "ethereum", window.web3.currentProvider)
    // window.ethereum = window.web3.currentProvider;

    if(window.okxwallet && window.okxwallet.isOkxWallet){
        state.isOkxWallet = window.okxwallet.isOkxWallet;

        // okxwallet.isOKExWallet=undefined;
        // okxwallet.isOkxWallet=undefined;
        // 连接
        okxwallet.on('connect',  (connectInfo) => {
            console.log(connectInfo,"connectInfo1")
        });
        // 断开
        okxwallet.on('disconnect',  (connectInfo) => {
            console.log(connectInfo,"connectInfo2")
        });
        // 帐户已更改
        okxwallet.on('accountsChanged',  (connectInfo) => {
            console.log(connectInfo,"connectInfo3")
            state.account = connectInfo[0];
        });
        // 当当前连接的链发生变化时，MetaMask 提供程序会发出此事件。
        okxwallet.on('chainChanged',  (connectInfo) => {
...
const onSign = async()=>{
    ...
    try {
        ...
        const msg =
            ...
            state.nonce;
        const sign = await okxwallet.request({
...
const onConnect = async () => {
    if(state.isOkxWallet){  //浏览器已安装钱包插件
        if(state.connect_state === 0){ //未获取地址  去获取地址
            console.log('onConnect',1)
            //登录的时候判断是不是在以太网
            
            // if((window.location.href.indexOf('heartx.art') >= 0) && window.ethereum.chainId != '0x1'){
            //      window.toast(`To claim $HNX, please switch to Ethereum's mainnet.`)
            //      return false;
            // }
            try {
                const newAccounts = await okxwallet.request({
        ...
        if(/ Edg\//i.test(ua)){
            jumpUrl="https://microsoftedge.microsoft.com/addons/detail/%E6%AC%A7%E6%98%93web3%E9%92%B1%E5%8C%85/pbpjkcldjiffchgbbndmhojiacbgflha";
        }else if(/ Firefox\//i.test(ua)){
            jumpUrl="https://addons.mozilla.org/zh-CN/firefox/addon/okexwallet/";
        }else{
            jumpUrl="https://chrome.google.com/webstore/detail/okx-wallet/mcohilncbfahbmgdjkbpemcciiolgcge";
        }
        pluginChangeEvent()
        window.open(jumpUrl);
    }
