...
<!--        <span v-if="connect_state === 0">{{$t('form.optional')}}</span>-->
      </div>
    </button>
  </div>
</template>
...
onMounted(() => {
  if(window.ethereum && window.ethereum.isMetaMask){
    state.isMetaMask = window.ethereum.isMetaMask;
    // 连接
